...
//NO USAR COLUMNS -> SIGUE PREPARADO PARA JQGRID CUANDO AHORA SE USA DATATABLES!!!!!!!!!!!!!
        ...
        _configureButton: function (button, defaultDialog, customDialog, settings) {
            ...
            if (!button.isInline) {
                $('[id=\'' + button.id + '\']').on('click', function () {
                    ...
                    //Filter
                    if(button.filter !== undefined){
						data.filter = JSON.stringify($("#" + button.filter).rup_form("formToJson"))
					}
        ...
        _getColumns: function ($grid, columns) {
            ...
            if (customNames === undefined) {
                for (let i = 0; i < colNumber; i++) {
                    ...
                    if (hidden || !hidden && (colModel[i].hidden === undefined || colModel[i].hidden === false)) {
                        let column = [];
