jQuery(function($) {
	...
	$('#persona').on('tableEditFormAddEditAfterShowForm', function(event, ctx) {
		$('[aria-describedby="persona_detail_div"]').css("width", "60%");
		$('[aria-describedby="persona_detail_div"]').css("left", "15%");
	});

	$('#persona').on('tableEditFormBeforeCallAjax', function() {
		bloquearPantalla($.rup.i18nParse($.rup.i18n.app, "comun.guardando"));
	});
	$('#persona').on('tableEditFormCompleteCallSaveAjax', desbloquearPantalla);
	$('#persona_detail_form').on('invalid-form.validate', desbloquearPantalla);

	$('#persona').on('tableEditFormAddEditBeforeShowForm', function(event, ctx) {
