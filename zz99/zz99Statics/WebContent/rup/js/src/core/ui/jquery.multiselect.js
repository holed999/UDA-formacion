...
(function ($) {
  ...
  $.widget('ech.multiselect', {
    ...
    _create: function _create() {
      ...
      button = (this.button = $('<button type="button"><span class="ui-icon ui-icon-triangle-1-s"></span></button>')).addClass('ui-multiselect ui-widget ui-state-default ui-corner-all').addClass(o.classes).attr({
        'title': el.attr('title'),
        'aria-haspopup': true,
        'tabIndex': el.attr('tabIndex')
      }).insertAfter(el),
          buttonlabel = (this.buttonlabel = $('<span></span>')).html(o.noneSelectedText).appendTo(button),
          menu = (this.menu = $('<div></div>')).attr('id', 'rup-multiCombo_' + $(this.element).attr('id')).addClass('ui-multiselect-menu ui-widget ui-widget-content ui-corner-all').addClass(o.classes).appendTo(o.appendTo),
    ...
    _bindEvents: function _bindEvents() {
      ...
      this.menu.on('click.multiselect', 'li.ui-multiselect-optgroup-label a', function (e) {
        ...
        var $this = $(this),
            ...
            nodes = $inputs.get(),
            label = $this.parent().text(); // trigger event and bail if the return is false
        ...
        if (!$(this).hasClass('ui-state-disabled')) {
          self.labels.removeClass('ui-state-hover'); //$(this).addClass('ui-state-hover').find('input').focus();
          //IE Fix: (Select - Disabled option)
        ...
        var $this = $(this),
            ...
            tags = self.element.find('option'); // bail if this input is disabled or the event is cancelled
    ...
    _toggleChecked: function _toggleChecked(flag, group) {
      ...
      var $inputs = group && group.length ? group : this.labels.find('input:enabled'),
          self = this; // toggle state on inputs
    ...
    open: function open(e) {
      var self = this,
          ...
          speed = this.speed,
          o = this.options,
          args = []; // bail if the multiselectopen event returns false, this widget is disabled, or is already open
      ...
      var $container = menu.find('ul').last(),
          effect = o.show,
          pos = o.positionMenuByOffset ? button.offset() : button.position(); // figure out opening effects/speeds
      ...
      $container.scrollTop(0).height(o.height); // position and show menu

      if ($.ui.position && !$.isEmptyObject(o.position)) {
        o.position.of = button || o.position.of; //al tener varios remotos que mantenga la posiciÃ³n.

        menu.show().position(o.position).hide(); // if position utility is not available...
      } else {
        menu.css({
      ...
      menu.show(); // select the first option
    ...
    close: function close() {
      ...
      var o = this.options,
          effect = o.hide,
          speed = this.speed,
          args = []; // figure out opening effects/speeds
...
//# sourceURL=webpack://rup/./src/core/ui/jquery.multiselect.js?